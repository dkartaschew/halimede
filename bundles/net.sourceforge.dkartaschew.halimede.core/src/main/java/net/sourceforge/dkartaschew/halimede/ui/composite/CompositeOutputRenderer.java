...
public class CompositeOutputRenderer extends Composite implements ICertificateOutputRenderer {
	 ...
	 */
	private static class HRDrawingStrategy implements AnnotationPainter.IDrawingStrategy {

		@Override
		public void draw(Annotation annotation, GC gc, StyledText textWidget, int offset, int length, Color color) {
			if (annotation == null || !annotation.getType().equals(HRAnnotation.TYPE)) {
				return;
			}
			if (gc != null) {
				...
				// Note: The following is based on org.eclipse.ui.texteditor.SourceViewerDecorationSupport.IBeamStrategy
				/*
				 * The length for HR's is always 0, which causes no redraw to occur in StyledText#redraw(int, int,
				 * boolean). We try to normally redraw at length of one, and up to the line start of the next line if
				 * offset is at the end of line. If at the end of the document, we redraw the entire document as the
				 * offset is behind any content.
				 */
				...
				if (ch == '\r' || ch == '\n') {
					...
					if (nextLine >= textWidget.getLineCount()) {
						/*
						 * Panic code: should not happen, as offset is not the last offset, and there is a delimiter
						 * character at offset.
						 */
						textWidget.redraw();
						return;
