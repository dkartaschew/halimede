...
public class CASettingsDialog extends Dialog {
	...
	protected DataBindingContext initDataBindings() {
		...
		IObservableValue<String> caDescriptionModel = PojoProperties.value("description", String.class).observe(model);
		 ...
		 */
		IObservableValue<Integer> expiryDaysWidget = WidgetProperties.spinnerSelection().observe(spinnerExpiry);
		IObservableValue<Integer> cexpiryDaysModel = PojoProperties.value("expiryDays", Integer.class).observe(model);
		UpdateValueStrategy<Integer, Integer> s1 = new UpdateValueStrategy<Integer, Integer>()
				.setAfterGetValidator(value -> {
					if (value <= 0) {
		...
		b = bindingContext.bindValue(expiryDaysWidget, cexpiryDaysModel, s1, null);
		ControlDecorationSupport.create(b, SWT.TOP | SWT.LEFT);

		/*
		 * Sig Algorithm Type
		 */
		IObservableValue<String> keyTypeWidget = WidgetProperties.comboSelection().observe(comboSigAlg);
		IObservableValue<SignatureAlgorithm> keyTypeModel = PojoProperties
				.value("signatureAlgorithm", SignatureAlgorithm.class).observe(model);
		IConverter<String, SignatureAlgorithm> convertStringToKeyType = IConverter.create(String.class,
				SignatureAlgorithm.class, (o1) -> SignatureAlgorithm.forAlgID(o1));
		UpdateValueStrategy<String, SignatureAlgorithm> s3 = UpdateValueStrategy.create(convertStringToKeyType);
		bindingContext.bindValue(keyTypeWidget, keyTypeModel, s3, null);

		/*
		 * Incremental Serial
		 */
		IObservableValue<Boolean> serialWidget = WidgetProperties.buttonSelection().observe(chkIncremental);
		IObservableValue<Boolean> serialModel = PojoProperties.value("incrementalSerial", Boolean.class).observe(model);
		bindingContext.bindValue(serialWidget, serialModel, null, null);

		/*
		 * Enable log
		 */
		IObservableValue<Boolean> logWidget = WidgetProperties.buttonSelection().observe(chkEnableLog);
		IObservableValue<Boolean> logModel = PojoProperties.value("enableLog", Boolean.class).observe(model);
